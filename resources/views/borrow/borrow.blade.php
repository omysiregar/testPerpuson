        ...
        @foreach ($borrows as $borrow)
            <div class="bg-white rounded-lg">
                <a href="{{ route('books.show', $borrow->book->slug) }}">
                    <div class="grid grid-cols-3 gap-4 p-4">
                        ...
                        <div class="col-span-2">
                            ...
                            <div class="mb-2">
                                <h1 class="font-semibold text-gray-800 text-sm">Maks. Dikembalikan</h1>
                                @if ($borrow->status == 'meminjam')
                                    <p
                                        ...
                                        {{ \Carbon\Carbon::parse($borrow->tgl_return)->format('d M Y') }}</p>
                                @else
                                    <p
                                        class="text-gray-800 text-sm bg-gradient-to-br from-green-500 to-green-600 inline-block text-white p-1 px-2 rounded mt-px">
                ...
                <div class="p-4">
                    @if ($borrow->status == 'meminjam')
                        <a href="{{ route('books.show', $borrow->book->slug) }}"
                            class="w-full bg-blue-600 mt-1 rounded-lg text-white font-medium p-3 text-sm inline-block text-center">
                            Baca Selengkapnya</a>
                        <form action="{{ route('borrow.update', $borrow->kode_peminjaman) }}" method="post">
                            ...
                            <button type="submit"
                                class="w-full bg-red-600 mt-1 rounded-lg text-white font-medium p-3 text-sm inline-block text-center"
                                onclick="return confirm('Konfirmasi Pengembalian Buku')"
                                @if ($borrow->status == 'dikembalikan') @disabled(true) @endif>Kembalikan</button>
                        </form>
                    @else
                        <button disabled
                            class="w-full bg-black/50 mt-1 rounded-lg text-white font-medium p-3 text-sm inline-block text-center">Sudah
                            Pernah Dibaca!</button>
                    @endif
                ...
                <table class="table-auto w-full">
                    <thead class="bg-gray-50 border-b-2 border-gray-200">
                        <tr>
                            ...
                            <th class="p-3 text-sm font-semibold tracking-wide text-left">Tgl Pengembalian Max</th>
                            <th class="p-3 text-sm font-semibold tracking-wide text-left">Dikembalikan</th>
                        ...
                        @foreach ($borrows as $borrow)
                            <tr>
                                ...
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap text-center">
                                    @php
                                        $tglReturn = \Carbon\Carbon::parse($borrow->tgl_return);
                                        $isLate = $tglReturn == \Carbon\Carbon::today();
                                    @endphp

                                    @if ($isLate && $borrow->status == 'meminjam')
                                        <div class="animate-bounce text-red-500" >
                                            {{ \Carbon\Carbon::parse($borrow->tgl_return)->format('d M Y') }}
                                        </div>
                                    @else
                                        {{ \Carbon\Carbon::parse($borrow->tgl_return)->format('d M Y') }}
                                    @endif
                                ...
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                    @if ($borrow->status == 'meminjam')
                                        --
                                    @else
                                        {{ empty($borrow->updated_at) ? '--' : $borrow->updated_at->format('d M Y') }}
                                    @endif
                                </td>
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                    <form action="{{ route('borrow.update', $borrow->kode_peminjaman) }}" method="post">
                                        ...
                                        <button type="submit"
                                            ...
                                            onclick="return confirm('Konfirmasi Pengembalian Buku')"
                                            @if ($borrow->status == 'dikembalikan') @disabled(true) @endif>Hentikan
                                            Peminjaman</button>
                                    </form>
                                </td>
                            </tr>
