...
<div id="sidebar" class="w-0 lg:w-1/6 transition-all duration-300 absolute lg:static z-10 translate-x-[-400rem] lg:translate-x-[0rem] h-screen bg-white">
    <div class="p-4 py-6 flex items-center justify-center">
        ...
        <div class="flex items-center">
            <span class="font-bold text-xl tracking-wide ml-3 text-[#303030]">Perpus Online</span>
        </div>
    </div>
    @can('isUser')
        ...
        <ul class="list-none list-inside px-5 mb-3">
            <li>
                <a href="{{ route('books.index') }}"
                    class="transition-all ease-in-out duration-300 flex items-center p-3 {{ Request::is('books*') ? 'shadow-lg shadow-blue-100 rounded-lg bg-gradient-to-br from-blue-400 to-blue-500' : '' }} mb-1">
                    @if (Request::is('books*'))
                    ...
                    @endif
                    <p class="ml-2 font-medium {{ Request::is('books*') ? 'text-white' : 'text-gray-700' }} ">Overview</p>
                </a>
            </li>
            <li>
            ...
            <li>
                <a href="{{ route('category.index') }}"
                    ...
                    @endif
                    <p class="ml-2 font-medium {{ Request::is('category*') ? 'text-white' : 'text-gray-700' }} ">Category
                    </p>
                </a>
            </li>
        </ul>
    ...
    @can('isAdmin')
        ...
        <ul class="list-none list-inside px-5 mb-3">
            <li>
                <a href="{{ route('books.index') }}"
                    class="transition-all ease-in-out duration-300 flex items-center p-3 rounded-lg {{ Request::is('books*') ? 'shadow-lg shadow-blue-200 rounded-lg bg-gradient-to-br from-blue-400 to-blue-500' : '' }} mb-1">
                    @if (Request::is('books*'))
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                            class="w-6 h-6 fill-white">
                            <path
                    ...
                    @endif
                    <p class="ml-2 font-medium {{ Request::is('books*') ? 'text-white' : 'text-gray-700' }} ">Books</p>
                </a>
            </li>
            <li>
                <a href="{{ route('borrow.index') }}" class="transition-all ease-in-out duration-300 flex items-center p-3 rounded-lg mb-1 {{ Request::is('borrow*') ? 'shadow-lg shadow-blue-200 rounded-lg bg-gradient-to-br from-blue-400 to-blue-500' : '' }}">
    ...
    <form action="/logout" method="POST">
        ...
        <button type="submit" class="absolute bottom-4 px-5 left-0 right-0">
            <div
                class="group bg-sky-100 p-3 rounded-lg flex items-center transition-all duration-300 transition-all duration-500 bg-gradient-to-br from-sky-100 to-sky-100 hover:from-blue-400 hover:to-blue-500 bg-size-200 bg-pos-0 hover:bg-pos-100">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor"
                    class="w-5 h-5 stroke-blue-700 transition-all duration-300 group-hover:stroke-white">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" />
                </svg>
                <p class="ml-2 font-medium text-blue-700 transition-all duration-300 group-hover:text-white">Logout</p>
            </div>
        </button>
    </form>
</div>
